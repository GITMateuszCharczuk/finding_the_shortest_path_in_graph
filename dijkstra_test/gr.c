    ...
    *rowcol = rc;
    ...
    {   
        for( k = 0; k < 4; k++)
        {
            ...
            // fprintf(stdout,"=%d %d %d{2}=",error_flag_1, error_flag_2, error_flag_3);
            error_flag_2 = tmp != COLON ? 1 : error_flag_2;
            // fprintf(stdout,"int%dint ",g[j].n[k]);
            //buff[0] = fgetc(in);
            // fprintf(stdout,"|%d|",i);
            for(i = 0; isspace(tmp) == 0; i++)
            {
                tmp = fgetc(in);
                buff[i] = tmp;
                // fprintf(stdout,"!%d!%c!",i,buff[i]);
            }
            // fprintf(stdout,"|%d|%c|",i-1,buff[i-1]);
            //fprintf(stdout,"[%s]",buff);
            // fprintf(stdout,"=%d %d %d{3}=",error_flag_1, error_flag_2, error_flag_3);
            error_flag_3 = buff[i-1] == NEW_LINE ? 1 : error_flag_3;
            // fprintf(stdout,"=%d %d %d{4}=",error_flag_1, error_flag_2, error_flag_3);
            EOFfaliture(buff[i-1], rc, g, buff, in);
            buff[i-1] = '\0';
            g[j].w[k] = is_good_d(buff, &error_flag_1);
            // fprintf(stdout,"DB%.16lfDB",g[j].w[k]);
            // fprintf(stdout,"=%d %d %d{5}=",error_flag_1, error_flag_2, error_flag_3);
            buff_cleaner(buff,i);
